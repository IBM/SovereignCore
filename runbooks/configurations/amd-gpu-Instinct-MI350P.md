...
```sh
podman login -u ${QUAY_USER} -p ${QUAY_PASS} ${QUAY} --tls-verify=false
```
...
mirror:
  ...
  additionalImages:
    ...
    - name: mirror.gcr.io/rocm/k8s-device-plugin:labeller-latest
...
spec:
  imageDigestMirrors:
    - mirrors:
        - registry-quay-quay-enterprise.apps.mgmt.<your-domain>/redhat/kmm
      source: registry.redhat.io/kmm
    - mirrors:
        - registry-quay-quay-enterprise.apps.mgmt.<your-domain>/redhat/rocm
...
spec:
  imageTagMirrors:
    - mirrors:
      ...
      source: docker.io/rocm
    - mirrors:
        - registry-quay-quay-enterprise.apps.mgmt.<your-domain>/redhat/library/busybox
      source: docker.io/library/busybox
    - mirrors:
        - registry-quay-quay-enterprise.apps.mgmt.<your-domain>/redhat/ubi9
...
```sh
# Apply the `CatalogSource` generated by `oc mirror`
...
spec:
  ...
  startingCSV: kernel-module-management.v2.4.1
EOF
```
...
# Grant privileged SCC to the amd-gpu-operator-node-labeller service account
oc adm policy add-scc-to-user privileged \
  -z amd-gpu-operator-node-labeller \
  -n openshift-amd-gpu

# Grant privileged SCC to the KMM module loader service account
oc adm policy add-scc-to-user privileged \
  -z amd-gpu-operator-kmm-module-loader \
  -n openshift-amd-gpu

# Grant privileged SCC to the device-plugin service account
oc adm policy add-scc-to-user privileged \
  -z amd-gpu-operator-kmm-device-plugin \
  -n openshift-amd-gpu

# Grant privileged SCC to the metrics-exporter service account
oc adm policy add-scc-to-user privileged \
  -z amd-gpu-operator-metrics-exporter \
  -n openshift-amd-gpu
...
controllerManager:
  manager:
    image:
      repository: ${QUAY}/redhat/rocm/gpu-operator
      tag: v1.5.1-beta.0
...
deviceConfig:
  spec:
    driver:
      ...
      useSourceImage: true
      version: "31.30"
      image: "${QUAY}/redhat/rocm/amdgpu-driver"
      imageRegistrySecret:
        name: quay-push-secret
    commonConfig:
      initContainerImage: ${QUAY}/redhat/library/busybox:1.36
      utilsContainer:
        image: ${QUAY}/redhat/rocm/gpu-operator-utils:v1.5.1-beta.0
        imagePullPolicy: IfNotPresent
    devicePlugin:
      devicePluginImage: ${QUAY}/redhat/rocm/k8s-device-plugin:rhubi-latest
      devicePluginImagePullPolicy: IfNotPresent
      nodeLabellerImage: ${QUAY}/redhat/rocm/k8s-device-plugin:labeller-latest
      nodeLabellerImagePullPolicy: IfNotPresent
    metricsExporter:
      enable: true
      image: ${QUAY}/redhat/rocm/device-metrics-exporter:v1.5.1-beta.0
      imagePullPolicy: IfNotPresent
    testRunner:
      enable: false
    configManager:
...
> **Note:** If `podman build` fails with a compression MIME type error, build the image
> using Docker (or another OCI-compatible builder) on a machine where the base image can
> be pulled cleanly, then push the resulting image directly to Sovereign Quay.
...
**Grant read permission to `sovereign-core-read` on the `rocm` organization**

The newly created `rocm` organization does not include `sovereign-core-read` by default,
causing image pull failures. In Sovereign Quay, open the `rocm` organization and grant
`sovereign-core-read` **Read** permission.
...
# Expected: True (model loading may take several minutes)
```
...
### 8-1. Create the Blueprint namespace and grant SCC

```sh
oc create namespace <blueprint-ns>
```
...
**Grant read permission to `sovereign-core-read` on the `fsi-blueprint` organization**

The newly created `fsi-blueprint` organization does not include `sovereign-core-read` by default,
causing image pull failures. In Sovereign Quay, open the `fsi-blueprint` organization and grant
`sovereign-core-read` **Read** permission.
